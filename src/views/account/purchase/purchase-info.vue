  ...
  <div class="flex flex-col h-full" v-loading="isLoading">
    ...
    <div class="flex-1 overflow-y-auto px-[1rem] space-y-4">
      <div
        ...
        v-for="(item, index) in order.buy_order"
        :key="index"
      >
          ...
          <template v-if="item.is_exception">
            <div class="text-[24px] space-y-[4px]">
              <div class="flex items-center justify-between">
                <p>
                  {{ t('orderDetail.updateTime') }}：{{ item.exception_time }}
                </p>
                <p class="text-[#ED2323]">
                  {{ item.is_exception ? t('orderDetail.isException') : '' }}
                </p>
              </div>
              <p class="text-[#ED2323]">
                <span>
              ...
              <div v-if="item.exception_images && item.exception_images.length">
                <div
                  class="mt-[20px] bg-gray-100 rounded-lg px-[24px] flex items-center"
                ...
                >
                  ...
                  <p
                    ...
                    v-if="item.is_exception"
                  >
                    {{ t('orderDetail.exceptionVoucher') }}
                  </p>
          ...
          <template v-if="item.status !== -1">
            <div class="text-[24px] space-y-[4px]">
              ...
              <div v-if="item.receipt_images && item.receipt_images.length">
                <div
                  class="mt-[10px] bg-gray-100 rounded-lg px-[24px] flex items-center"
                  v-if="item.status === 4"
